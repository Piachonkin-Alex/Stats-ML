...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    x_1_square = torch.sum(torch.pow(x_1, 2), dim=1).reshape(x_1.shape[0], 1)
    x_2_square = torch.sum(torch.pow(x_2, 2), dim=1).reshape(x_2.shape[0], 1)
    distances = torch.exp(-(x_1_square + x_2_square.T - 2 * (x_1 @ x_2.T)) / (2 * sigma))
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(torch.max(torch.zeros_like(labels),  1 - scores * labels))
