...
class KNearestNeighbor:
    ...
    def compute_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        first = np.matrix(np.power(X, 2).sum(axis=1))
        second = np.matrix(np.power(self.X_train, 2).sum(axis=1)).T
        third = 2 * (self.X_train @ X.T)
        dists = np.sqrt((first + second) - third).T
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
